#!/bin/bash
# 10_run_nsl_baseline.sh — compute NSL for a specific (model, dataset)
# across all datasplits and languages defined in constants.py.
#
# Usage:
#   bash 10_run_nsl_baseline.sh --model llama_3.1_8B --dataset global_mmlu
#   bash 10_run_nsl_baseline.sh --model qwen3_8B --dataset mkqa --dry-run
#
# Logs: each run appends to logs/nsl_<model>_<dataset>_<split>_<lang>.log
...
if [[ -z "$MODEL" || -z "$DATASET" ]]; then
    echo "Usage: bash 10_run_nsl_baseline.sh --model <model> --dataset <dataset> [--dry-run]"
    exit 1
fi
...
while IFS=' ' read -r model dataset split lang; do
    COUNT=$(( COUNT + 1 ))
    LOG="logs/nsl_${model}_${dataset}_${split}_${lang}.log"

    CMD="python3 ../src/nsl_baseline.py compute_nsl \
        --model-name     $model \
