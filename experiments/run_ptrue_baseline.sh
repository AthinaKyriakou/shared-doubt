#!/bin/bash
# 10_run_ptrue_baselines.sh — compute P(True) for a specific (model, dataset)
# across all datasplits and languages defined in constants.py.
#
# Usage:
#   bash run_p_true.sh --model llama_3.1_8B --dataset global_mmlu
#   bash run_p_true.sh --model qwen3_8B --dataset mkqa --dry-run
#
# Logs: each run appends to logs/p_true_<model>_<dataset>_<split>_<lang>.log
...
if [[ -z "$MODEL" || -z "$DATASET" ]]; then
    echo "Usage: bash run_p_true.sh --model <model> --dataset <dataset> [--dry-run]"
    exit 1
fi
...
while IFS=' ' read -r model dataset split lang; do
    COUNT=$(( COUNT + 1 ))
    LOG="logs/p_true_${model}_${dataset}_${split}_${lang}.log"

    CMD="python3 ../src/ptrue_baseline.py compute_p_true \
        --model-name     $model \
